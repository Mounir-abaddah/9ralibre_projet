...
router.post('/save-cours/:coursId', authMiddleware, async(req,res)=>{
    try{
        ...
        return res.status(200).send({
            success:true,
            isSaved: newIsSaved,
            message: newIsSaved ? "Cours enregistré" : "Cours supprimé des enregistrements"
        })
    }catch(err){
        console.error("Erreur save cours:", err);
        return res.status(500).send({message:"Une erreur est survenue lors de l'enregistrement du cours",success:false,err})
    }
});
...
router.get('/get-saved-cours', authMiddleware, async(req,res)=>{
    try{
        ...
        return res.status(500).send({message:"Une erreur est survenue lors de la récupération des cours enregistrés",success:false,err})
    }
});
