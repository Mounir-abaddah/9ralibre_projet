...
const Navbar = () => {
  ...
  const refreshConversations = async () => {
    if (!data?.id) return;
    const res = await axios.get(`${apiUrl}/chat/my-conversation`, {
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-3">
            {data ? (
              ...
              <>
                ...
                <Link to="/inscription">
                  <Button className="cursor-pointer bg-amber-400 hover:bg-amber-500">
                    {t("nav.inscription")}
                  </Button>
                </Link>
                <Link to="/prof-connexion">
                  <Button className="cursor-pointer bg-cyan-400 hover:bg-cyan-500">
                    {t("nav.teacherSpace")}
                  </Button>
                </Link>
              </>
      ...
      <Sheet open={isCompactNav && open} onOpenChange={setOpen}>
        <SheetContent
          side="right"
          className="z-[10000] w-[85%] max-w-sm overflow-y-auto px-5 py-6 sm:w-[420px]"
        >
    ...
    <div className="flex h-full flex-col justify-between p-3">
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-4">
          ...
          <Link to={`/cours/${data?.niveaux}`} onClick={onNavigate}>
            {t("nav.courses")}
          </Link>
          <Link to={`/videos/${data?.niveaux}`} onClick={onNavigate}>
            {t("nav.videos")}
          </Link>
          {!data && (
            <>
              ...
              <Link to="/prof-connexion" onClick={onNavigate}>
                {t("nav.iAmTeacher")}
              </Link>
            </>
          )}
      ...
      {!data ? (
        <div className="flex flex-col gap-2">
          ...
          <Link to="/inscription">
            <Button className="w-full cursor-pointer bg-cyan-500 hover:bg-cyan-600">
              {t("nav.inscription")}
            </Button>
          </Link>
        </div>
