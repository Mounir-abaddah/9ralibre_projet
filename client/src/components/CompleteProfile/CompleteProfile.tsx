  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (axios.isAxiosError(err) && err.response) {
        toast.error(
          err.response.data.message || t("completeProfile.toast.error"),
        );
      }
    } finally {
      setLoading(false);
      ...
      <DialogContent className="max-w-3xl w-full">
        ...
        <DialogHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <span className="flex items-center gap-2 rounded-md border-b-2 border-sky-300 text-lg font-bold">
            {step === 0
              ? t("completeProfile.steps.personalInfo")
              : step === 1
                ? t("completeProfile.steps.uploadPhoto")
                : t("completeProfile.steps.studyLevel")}
          </span>
        ...
        <form
          className="flex w-full flex-col justify-around gap-5"
        ...
        >
          ...
          <div className="flex w-full justify-end gap-2">
            ...
            <Button
              type="submit"
              disabled={loading}
              className="cursor-pointer bg-amber-400 text-slate-700 hover:bg-amber-300"
            >
              {loading
                ? t("completeProfile.buttons.sending")
                : step === 2
                  ? t("completeProfile.buttons.finish")
                  : t("completeProfile.buttons.next")}
            </Button>
          </div>
        </form>
      </DialogContent>
