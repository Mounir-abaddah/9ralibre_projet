        ...
        <div className="flex w-full flex-col space-y-4">
            ...
            <div className="flex w-full flex-wrap justify-between space-y-4 md:space-y-0 lg:space-y-0">
                ...
                <div className="flex gap-2">
                    ...
                    <div>
                        <Button onClick={()=>handleFollowProfesseur(videos.professeur._id)} className="cursor-pointer bg-amber-500 hover:bg-amber-600">{likeProfesseur ? t("common.alreadySubscribed") : t("common.subscribe")}</Button>
                    </div>
                </div>
                ...
                <div className="flex gap-2">
                    ...
                    <Button variant={'outline'} className="hidden cursor-pointer md:flex lg:flex"><Share />{t("common.share")}</Button>
                    <Button variant="outline" className={`hidden cursor-pointer items-center gap-1 transition-all duration-200 md:flex lg:flex ${isSaved ? 'bg-amber-50 text-amber-500' : ''}`} onClick={handleSave}><Bookmark color={isSaved ? '#FF9500' : '#000'} fill={isSaved ? '#FF9500' : '#fff'} />{isSaved ? t("common.saved") : t("common.save")} </Button>
                    <Popover>
                        <PopoverTrigger asChild>
                            <Button variant="outline" className="cursor-pointer"><EllipsisVertical /></Button>
                        </PopoverTrigger>
                        <PopoverContent className="w-42 space-y-2">
                            <Button variant="outline" className="flex w-full cursor-pointer md:hidden lg:hidden"><Share />{t("common.share")}</Button>
                            <Button variant="outline" className={`flex w-full cursor-pointer items-center gap-1 transition-all duration-200 md:hidden lg:hidden ${isSaved ? 'bg-amber-50 text-amber-500' : ''}`} onClick={handleSave}><Bookmark color={isSaved ? '#FF9500' : '#000'} fill={isSaved ? '#FF9500' : '#fff'} />{isSaved ? t("common.saved") : t("common.save")}</Button>
                            <Button variant="outline" className="w-full cursor-pointer" onClick={() => setOpenReportVideo(true)}><Flag />{t("common.report")}</Button>
                        </PopoverContent>
                    </Popover>
                </div>
            </div>
            ...
            <div className="rounded-md border p-2">
                <span>{t("video.viewsCount", { count: videos.views })} . {formatDistanceToNow(new Date(videos.createdAt),{addSuffix:true,locale:fr})}</span>
