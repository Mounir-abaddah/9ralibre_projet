...
function LandingPage({ user }: { user: typedata | null }) {
  ...
  const quickLinks = user && n
    ? [
        ...
        { to: `/Save/${n}`,       label: t("home.quickLinks.saves"),          desc: t("home.quickLinks.savesDesc"),          icon: BookMarked },
      ]
    : [];

  const features = [
    { icon: Library,       title: t("home.features.library.title"),    desc: t("home.features.library.desc") },
    { icon: Video,         title: t("home.features.videos.title"),     desc: t("home.features.videos.desc") },
    { icon: GraduationCap,title: t("home.features.quiz.title"),        desc: t("home.features.quiz.desc") },
    { icon: MessageCircle, title: t("home.features.messaging.title"),  desc: t("home.features.messaging.desc") },
    { icon: Bookmark,      title: t("home.features.bookmarks.title"),  desc: t("home.features.bookmarks.desc") },
    { icon: Calendar,      title: t("home.features.calendar.title"),   desc: t("home.features.calendar.desc") },
    { icon: BookOpen,      title: t("home.features.stories.title"),    desc: t("home.features.stories.desc") },
  ];

  const steps = [
    { n: "01", title: t("home.steps.step1.title"), text: t("home.steps.step1.text") },
    { n: "02", title: t("home.steps.step2.title"), text: t("home.steps.step2.text") },
    { n: "03", title: t("home.steps.step3.title"), text: t("home.steps.step3.text") },
  ];
  ...
  const stats = [
    ...
    { k: "3", l: t("home.stats.formats.label"), s: t("home.stats.formats.sub") },
    { k: "1", l: t("home.stats.account.label"), s: t("home.stats.account.sub") },
  ];
    ...
    <div className="w-full">
        ...
        <div className="grid gap-12 px-4 py-16 md:grid-cols-2 md:items-center md:gap-10 md:py-20 lg:px-8 lg:py-24">
          <div>
            ...
            <p className="mt-6 max-w-xl text-lg leading-relaxed text-pretty text-zinc-600 dark:text-zinc-400">
              {user ? (
                <>
                  ...
                  {n ? (
                    <>
                      {" "}— {t("home.hero.level_label")} <span className="font-semibold">{n}</span>.
                    </>
                  ) : null}{" "}
            ...
            <div className="mt-10 flex flex-wrap gap-4">
              {user && n ? (
                <>
                  <Button asChild size="lg" className="h-12 rounded-lg bg-amber-500 px-8 text-base font-semibold text-zinc-950 shadow-none hover:bg-amber-600">
                    <Link to={`/Dashboard/${n}`}>
            ...
            <p className="mt-8 text-sm text-zinc-500">
              ...
              <Link to="/prof-connexion" className="font-semibold text-amber-700 underline-offset-4 hover:underline dark:text-amber-500">
      ...
      <section className="border-b border-zinc-200 bg-zinc-100 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="mx-auto grid max-w-6xl gap-8 px-4 py-12 sm:grid-cols-3 sm:gap-0 lg:px-8">
          {stats.map((item, i) => (
            ...
            >
              <p className="text-4xl font-bold text-zinc-900 tabular-nums dark:text-zinc-50">{item.k}</p>
              <p className="mt-1 text-sm font-semibold tracking-wide text-zinc-700 uppercase dark:text-zinc-300">{item.l}</p>
              <p className="mt-1 text-xs text-zinc-600 dark:text-zinc-500">{item.s}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="mx-auto max-w-6xl px-4 py-16 lg:px-8 lg:py-20">
          ...
          <ul className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {features.map(({ icon: Icon, title, desc }) => (
              <li key={title} className="rounded-2xl border border-zinc-200 bg-zinc-50/80 p-6 dark:border-zinc-800 dark:bg-zinc-900/80">
                <span className="flex h-11 w-11 items-center justify-center rounded-xl border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-800">
                  <Icon className="h-5 w-5 text-amber-600 dark:text-amber-500" aria-hidden />
                </span>
      ...
      <section className="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="mx-auto max-w-6xl px-4 py-16 lg:px-8 lg:py-20">
          ...
          <ol className="mx-auto mt-14 grid max-w-4xl gap-10 md:grid-cols-3 md:gap-8">
            {steps.map((step) => (
              <li key={step.n} className="relative border-l-4 border-amber-500 pl-6 dark:border-amber-600">
                <span className="text-xs font-bold text-amber-700 dark:text-amber-500">{step.n}</span>
                <h3 className="mt-2 text-lg font-semibold text-zinc-900 dark:text-zinc-100">{step.title}</h3>
                <p className="mt-2 text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">{step.text}</p>
              </li>
            ))}
          </ol>
        </div>
          ...
          <div className="flex flex-col items-center gap-10 lg:flex-row lg:items-start lg:justify-between lg:gap-16">
            <div className="max-w-xl text-center lg:text-left">
              ...
              <div className="mt-8 flex flex-wrap justify-center gap-2 lg:justify-start">
                {niveaux.map((n) => (
                  <span key={n} className="inline-flex items-center rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-2 text-sm font-semibold text-zinc-800 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200">
                    {n}
                  </span>
                ))}
              </div>
            ...
            <div className="flex w-full max-w-md flex-col gap-4 rounded-2xl border border-zinc-200 bg-zinc-50 p-6 dark:border-zinc-800 dark:bg-zinc-900">
              <div className="flex gap-3">
                ...
                <div>
                  <p className="font-semibold text-zinc-900 dark:text-zinc-100">{t("home.trust.title")}</p>
                  <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">{t("home.trust.subtitle")}</p>
                </div>
              </div>
      ...
      <section className="bg-zinc-900 text-zinc-50 dark:bg-black">
        <div className="mx-auto max-w-6xl px-4 py-16 text-center lg:px-8 lg:py-20">
          {user && n ? (
            <>
              <h2 className="text-2xl font-bold tracking-tight sm:text-3xl">{t("home.cta.userTitle")}</h2>
              <p className="mx-auto mt-4 max-w-lg text-sm text-zinc-400">{t("home.cta.userDesc")}</p>
              <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
                <Button asChild size="lg" className="h-12 rounded-lg bg-amber-500 px-8 text-base font-semibold text-zinc-950 hover:bg-amber-400">
                  <Link to={`/Dashboard/${n}`}>{t("home.cta.openDashboard")}</Link>
                </Button>
                <Button asChild variant="outline" size="lg" className="h-12 rounded-lg border-zinc-600 bg-transparent px-8 text-base font-semibold text-zinc-50 hover:bg-zinc-800">
            ...
            <>
              <h2 className="text-2xl font-bold tracking-tight sm:text-3xl">{t("home.cta.guestTitle")}</h2>
              <p className="mx-auto mt-4 max-w-lg text-sm text-zinc-400">{t("home.cta.guestDesc")}</p>
              <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
                <Button asChild size="lg" className="h-12 rounded-lg bg-amber-500 px-8 text-base font-semibold text-zinc-950 hover:bg-amber-400">
    ...
    <footer className="border-t border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-4 lg:gap-8">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="mt-4 flex gap-3">
              {[
                { to: "https://facebook.com/9ralibre",  label: "Facebook",  Icon: Facebook },
                { to: "https://instagram.com/9ralibre", label: "Instagram",  Icon: Instagram },
                { to: "https://linkedin.com/9ralibre",  label: "LinkedIn",   Icon: Linkedin },
              ].map(({ to, label, Icon }) => (
                <Link
                  key={label}
                  to={to}
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2 text-sm">
              <li>
                <Link to="/" className="text-zinc-600 transition-colors hover:text-amber-700 dark:text-zinc-400 dark:hover:text-amber-500">
              ...
              {user && n ? (
                ...
                <>
                  <li><Link to="/connexion"  className="text-zinc-600 transition-colors hover:text-amber-700 dark:text-zinc-400 dark:hover:text-amber-500">{t("nav.connexion")}</Link></li>
                  <li><Link to="/inscription" className="text-zinc-600 transition-colors hover:text-amber-700 dark:text-zinc-400 dark:hover:text-amber-500">{t("nav.inscription")}</Link></li>
                </>
              )}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2 text-sm">
              {user && n && (
                <>
                  <li><Link to={`/Quiz/${n}`} className="text-zinc-600 transition-colors hover:text-amber-700 dark:text-zinc-400 dark:hover:text-amber-500">{t("home.footer.resources.quiz")}</Link></li>
                  <li><Link to={`/Save/${n}`} className="text-zinc-600 transition-colors hover:text-amber-700 dark:text-zinc-400 dark:hover:text-amber-500">{t("home.footer.resources.saves")}</Link></li>
                </>
              )}
              <li><Link to="/prof-connexion" className="text-zinc-600 transition-colors hover:text-amber-700 dark:text-zinc-400 dark:hover:text-amber-500">{t("home.footer.resources.teacherSpace")}</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <p className="mt-4 text-sm text-zinc-600 dark:text-zinc-400">
              ...
              <a href="mailto:contact@9ralibre.com" className="border-b-2 border-b-cyan-400">
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-zinc-200 pt-8 text-center text-xs text-zinc-500 sm:flex-row sm:text-left dark:border-zinc-800 dark:text-zinc-500">
          <p>© {new Date().getFullYear()} 9ralibre. {t("home.footer.rights")}</p>
